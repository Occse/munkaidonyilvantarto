        ...
        mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(this, userPreferencesTask -> {
            if (userPreferencesTask.isSuccessful()) {
                ...
                Objects.requireNonNull(user).sendEmailVerification()
                        .addOnCompleteListener(this, (OnCompleteListener) emailSender -> {
                            if (emailSender.isSuccessful()) {
                                Toast.makeText(RegisterActivity.this, "Megerősítő email kiküldve a következő címre: " + user.getEmail(), Toast.LENGTH_SHORT).show();
                            } else {
                                Log.e(this.getLocalClassName(), "sendEmailVerification", emailSender.getException());
                                Toast.makeText(RegisterActivity.this, "Nem sikerült kiküldeni a megerősítő emailt.", Toast.LENGTH_SHORT).show();
                            }
                        });
                finish();
                showMainUser();
